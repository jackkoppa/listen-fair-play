...
const checkServer = () => {
  http.get(searchServerUrl, (res) => {
    ...
    console.error(`Search dev server not running or unreachable at ${searchServerUrl}. Please run \`pnpm search-lambda:dev:local\` in a separate terminal. Error: ${err.message}`);
    process.exit(1);
  });
};
